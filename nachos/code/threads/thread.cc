...
Thread::Thread(char* threadName)
{
    ...
    //Initialiser le tableau des fichier ouverts 
    for (int i = 0 ; i < 10; i++) 
    {
        openFiles[i] = NULL;
    }
...
int Thread::AddOpenFile(OpenFile* openFile) 
{
    for (int i = 0; i < 10; i++) 
    {
        if (openFiles[i] == NULL) 
        {
            ...
            printf("OVERTURE DE [%i]\n", i + 2);           
            return i + 2;
        }
    }
    
    printf("Impossible d'ajouter le fichier car le tableau est plein");
    return -1;
}

void Thread::RemoveAllOpenFiles() 
{
